...
const handleUpdateProfile = async (e) => {
  ...
  if (profileData.email !== originalEmail && !showOTPInput) {
    setIsUpdatingProfile(true);
    try {
      await axios.post('http://localhost:8000/api/accounts/send-otp/', { email: profileData.email });
      setShowOTPInput(true);
      alert('An OTP has been sent to your new email. Please enter it to confirm.');
    } catch (error) {
      console.error('Failed to send OTP:', error);
      alert('Failed to send OTP to new email. Make sure it is valid.');
    } finally {
        ...
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content border-0 shadow-lg" style={{ borderRadius: '24px', overflow: 'hidden' }}>
            <div className="modal-header border-0 bg-light p-4">
              ...
              <button type="button" className="btn btn-link text-dark p-0" onClick={() => setShowRewardsModal(false)}>
                <X size={24} />
              </button>
            </div>
        ...
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content border-0 shadow-lg" style={{ borderRadius: '24px', overflow: 'hidden' }}>
            <div className="modal-header border-0 bg-light p-4">
              ...
              <button type="button" className="btn btn-link text-dark p-0" onClick={() => setShowNotificationsModal(false)}>
                <X size={24} />
              </button>
            </div>
            <div className="modal-body p-5 bg-white text-center">
              ...
              <h6 className="fw-bold text-dark mb-1">No recent notifications</h6>
              <p className="small text-muted mb-0">Updates on your activity will appear here soon.</p>
            </div>
            <div className="modal-footer border-0 bg-white p-3 justify-content-center">
              <button
                  ...
                  <div className="d-flex justify-content-center mb-2">
                    <div className="position-relative">
                      <div className="rounded-circle overflow-hidden bg-success bg-opacity-10 shadow-sm" style={{ width: '80px', height: '80px' }}>
                        <img
                          src={avatarFile ? URL.createObjectURL(avatarFile) : (profileData.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${profileData.name || 'User'}`)}
                          alt="Profile Avatar"
